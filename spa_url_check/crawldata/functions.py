...
def is_domain(url_request,url_response):
    url_request=str(url_request).lower()
    url_response=str(url_response).lower()
    domain0 = (urlparse(url_request).netloc).replace('www.','')
    domain1 = (urlparse(url_response).netloc).replace('www.','')
    if domain0==domain1:
        return True
    else:
        return False
