...
class rocky(scrapy.Spider):
    ...
    ptag = "&page=1"
    start_url = [
        "https://almalinux.discourse.group/directory_items?period=all{ptag}&order=likes_received"
        # "https://forums.rockylinux.org/directory_items.json?order=likes_received{ptag}&period=all"
    ]
    second = "https://almalinux.discourse.group/u/{username}"
    user_agents = [
    ...
    headers = {
        ...
        'referer': 'https://almalinux.discourse.group/u',
    ...
    def main_page(self, response):
        ...
        for user in users:
            ...
            items['Name'] = name
            yield Request(
                f"https://almalinux.discourse.group/u/{username}/card.json",
